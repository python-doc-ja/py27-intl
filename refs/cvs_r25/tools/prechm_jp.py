...
"""
    ...
    Modified for pythondoc-jp Japanese Document 2.3, 04-Apr-2004 by
    Yasushi Masuda .
...
usage_mode = '''
使用法: precmh_jp.py [-c] [-k] [-p] [-v 1.5[.x]] ファイル名
    -c: .hhc ファイル (目次) を作成しない
    -k: .hhk ファイル (索引) を作成しない
    -p: .hhp ファイル (プロジェクトファイル) を作成しない
    -v 2.3jp: 2.3jp 用のドキュメントを作成する
        (Python 2.3/2.4 ドキュメント和訳: 2.3jp/2.4jp をサポートしています)
'''
...
Language=0x411 (日本語)
Title=Python %(version)s ドキュメント

[WINDOWS]
%(arch)s="Python %(version)s ドキュメント","%(arch)s.hhc","%(arch)s.hhk",\
...
supported_libraries = {  
    ...
    [
        Book('.', 'メインページ', 'index'),
        Book('.', 'グローバルモジュールインデクス', 'modindex'),
        Book('whatsnew', "更新情報", 'index', 'contents'),
        Book('tut','チュートリアル','tut','node2'),
        Book('lib','ライブラリリファレンス','lib','contents','genindex'),
        Book('ref','言語リファレンス','ref','contents','genindex'),
        Book('mac','Macintosh リファレンス','mac','contents','genindex'),
        Book('ext','拡張と埋め込み','ext','contents'),
        Book('api','Python/C API','api','contents','genindex'),
        Book('doc','ドキュメントの作成','doc','contents'),
        Book('inst','Python モジュールのインストール', 'inst', 'index'),
        Book('dist','Python モジュールの配布', 'dist', 'index'),
    ],
