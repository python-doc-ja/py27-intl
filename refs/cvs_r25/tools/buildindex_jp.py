...
REPLACEMENTS = [
    ...
    (re.compile(r"\\ABC\b"), u"ABC"),
    (re.compile(r"\\ASCII\b"), u"ASCII"),
    (re.compile(r"\\Cpp\b"), u"C++"),
    (re.compile(r"\\EOF\b"), u"EOF"),
    (re.compile(r"\\NULL\b"), u"NULL"),
    (re.compile(r"\\POSIX\b"), u"POSIX"),
    (re.compile(r"\\UNIX\b"), u"Unix"),
    # deal with turds left over from LaTeX2HTML
    (re.compile(r"<#\d+#>"), u""),
    ]

ascii_ident_letters = list(unicode(string.ascii_letters + "_"))


class Node:
    # __rmjunk = re.compile(u"<#\d+#>", re.UNICODE)

    continuation = 0

    def __init__(self, link, str, seqno):
        str = unicode(str, 'euc-jp')
        self.links = [link]
        self.seqno = seqno
        # remove <#\d+#> left in by moving the data out of LaTeX2HTML
        # str = self.__rmjunk.sub(u'', str)
...
def split_entry(str, which):
    stuff = []
    parts = str.split(u'!')
    parts = [part.split(u'@') for part in parts]
    for entry in parts:
        if len(entry) != 1:
            key = entry[which]
        else:
...
_rmtt = re.compile(ur"""(.*)<tt(?: class=['"][a-z0-9]+["'])?>(.*)</tt>(.*)$""", re.IGNORECASE|re.UNICODE)
_rmparens = re.compile(ur"\(\)", re.UNICODE)

def split_entry_key(str):
    parts = split_entry(str, 0)
...
_rmkey = re.compile(u"""[^\^! ][^!@ ]*@""", re.UNICODE)

def split_entry_text(str):
    (str, nrep) = _rmkey.subn('', str)
    if u'<' in str:
        m = _rmtt.match(str)
        if m:
...
def get_first_letter(s):
    if s.startswith(u"<tex2html_percent_mark>"):
        return u"%"
    else:
        return trim_ignored_letters(s)[0]


def split_letters_o(nodes):
    letter_groups = []
    if nodes:
        group = []
        append = group.append
...
def split_letters(nodes):
    ...
    if nodes:
        group = []
        append = group.append
        letter = get_first_letter(nodes[0].key[0])
        letter_groups.append((letter, group))
        for node in nodes:
            nletter = get_first_letter(node.key[0])
            if letter != nletter:
                letter = nletter
                group = []
                letter_groups.append((letter, group))
...
def group_symbols(groups):
    entries = []
    while groups[0][0] not in ascii_ident_letters:
        entries += groups[0][1]
        del groups[0]
    if entries:
        groups.insert(0, (unicode("記号", 'euc-jp'), entries))
...
def format_column(nodes):
    ...
    for node in nodes:
        ...
        for i in range(min(len(current), len(previous))):
            ...
            count = i + 1
        if count > level:
            append(u"<dl compact='compact'>" * (count - level) + u"\n")
            level = count
        elif level > count:
            append(u"\n")
            append(level * DL_LEVEL_INDENT)
            append(u"</dl>" * (level - count))
            level = count
        # else: level == count
...
def format_nodes(nodes, columns=1):
    ...
    if columns > 1:
        ...
        append(u'<table width="100%"><tr valign="top">')
...
def format_letter(letter):
    if letter == u'.':
        lettername = unicode(". (ドット)", 'euc-jp')
    elif letter == u'_':
        lettername = unicode("_ (アンダースコア)", 'euc-jp')
    else:
...
def format_html_letters(nodes, columns, group_symbol_nodes):
    ...
    for letter, nodes in letter_groups:
        s = u"<b><a href=\"#letter-%s\">%s</a></b>" % (letter, letter)
        items.append(s)
    s = [u"<hr /><center>\n%s</center>\n" % u" |\n".join(items)]
    for letter, nodes in letter_groups:
...
def main():
    ...
    nodes = []
    for fn in args:
        nodes = nodes + load(open(fn, 'rb'))
    num_nodes = len(nodes)
    html = process_nodes(nodes, columns, letters, group_symbol_nodes)
    program = os.path.basename(sys.argv[0])
    if ofn == "-":
        ef = codecs.EncodedFile(sys.stdout, 'unicode_internal', 'euc_jp', 'replace')
        # sys.stdout.write(html.encode('euc_jp', 'xml'))
        ef.write(html)
        sys.stderr.write("\n%s: %d index nodes" % (program, num_nodes))
    else:
        open(ofn, "wb").write(html)
