\chapter{MacintoshでPythonを利用する \label{using}} % Using Python on a Macintosh
...
の他の \UNIX プラットフォームと同じです。ただ、IDEやパッケージマネージャ
...
Mac OS 9 以前のバージョン上での Python は、\UNIX{} や Windows 上の 
Python とはかなり異なります。しかしこのマニュアルでは取り扱いません。と
いうのも、Python 2.4 以降ではこのプラットフォームがサポート対象外となっ
ているからです。Mac OS 9 用の最新の 2.3 系リリースのインストーラやドキュ
メントが \url{http://www.cwi.nl/\textasciitilde jack/macpython}
にあります。

\section{MacPythonの入手とインストール \label{getting-OSX}} % Getting and Installing MacPython
...
場合は、MacPythonのウェブサイト \url{http://www.cwi.nl/\textasciitilde jack/macpython}
から \program{MacPython for Panther additions} をインストールしなければ
なりません。
...
%%     the Finder; and the Package Manager.
    \file{Applications} フォルダ下の \file{MacPython-2.3} フォルダ。
このフォルダの中には、 PythonIDE 統合開発環境、ファインダから
ダブルクリックして Python スクリプトを起動するための PythonLauncher、
Package Manager が入っています。
...
%%     \file{/usr/local/lib/python}.
    ...
    \file{/usr/local/bin/python} にインストールされます。ただし、
    通常作成される\file{/usr/local/lib/python} はできません。
...
\file{/usr/bin/python} にインストールされています。原則として、これらを
...
他の \UNIX{} プラットフォーム上で動作する Python について詳しいなら、
\UNIX{} シェルからの Python スクリプトの実行を説明している節を読むのが
よいでしょう。

\subsection{Python スクリプトの実行方法} % How to run a Python script
...
が最良の方法です。章 \ref{IDE} を見るか、IDE が起動しているならヘルプ
メニューから呼び出せる Apple ヘルプビューア書類の IDE 入門を読みながら 
IDE に触れてみてください。
...
なら、まずはスクリプトを書くエディタが必要になります。
Mac OS X には、\program{vim} や \program{emacs} のような、様々な標準の
\UNIX{} コマンドラインエディタがついてきます。もっと Mac らしいエディタ
を使いたければ、 Bare Bones Software 
(\url{http://www.barebones.com/products/bbedit/index.shtml}) の 
\program{BBEdit} か \program{TextWrangler} を選ぶと良いでしょう。
\program{AppleWorks} や、ASCII 形式でファイルを保存できるその他のワード
プロセッサ、あるいは OS X に同梱されている \program{TextEdit} でもかまい
ません。
...
パスに \file{/usr/local/bin} 含まれるようにしておかなければなりません。
...
%%     and double-click it.
      Finder の情報ウィンドウで、作成したスクリプト (又はその他の 
      \file{.py} スクリプト) を開くためのデフォルトのアプリケーション
      として \program{PythonLauncher} を選択して、スクリプトを
      ダブルクリックします。
\end{itemize}
...
\subsection{GUI つきのスクリプトの実行 \label{osx-gui-scripts}} % Running scripts with a GUI
...
Mac OS X には、一つだけ知っておかねばならないクセがあります: 
Aqua ウィンドウマネージャとやり取りするような (すなわち、何らかの GUI
を持つような) プログラムは、特殊な方法で起動せねばならないのです。
GUIを持ったスクリプトを実行するには \program{python} の代わりに
\program{pythonw} を使ってください。

\subsection{設定} % configuration
...
Finder が起動時に \file{.profile} や \file{.cshrc} を読まないためです。
Finder から起動するプログラム向けに環境変数を設定したければ、
\file{\textasciitilde /.MacOSX/environment.plist} ファイルを作成して
...
\section{統合開発環境 \label{IDE}} % The IDE
...
\subsection{``Python Interactive''ウィンドウを使う} % Using the ``Python Interactive'' window
...
\subsection{Pythonスクリプトを書く \label{IDEwrite}} % Writing a Python Script
...
\program{Python IDE} の上にドロップすると、ファイルを編集用に開きます。
...
デフォルトでは、ファイルの\program{Python IDE} をクリエータコードに
して保存します。従って、ファイルのアイコンをダブルクリックすると
ファイルを編集用に開きます。この動作を変更して、
\program{PythonLauncher} で開いて実行するようしたいと思う場合も
あるでしょう。そうするには、単に「save options」から
「PythonLauncher」を選ぶだけです。このオプションは
アプリケーションではなく\emph{ファイル} に関連付けられているので注意
してください。

\subsection{統合開発環境の中からスクリプトを実行する
            \label{IDEexecution}} % Executing a script from within the IDE
...
習慣通りに \samp{if __name__ == "__main__":} と書いても、スクリプトは
デフォルトでは「__main__」に \emph{ならない} ことに注意しておきましょう。
そういう風に動作させるには、ドキュメントウィンドウの一番右上の小さ
な黒い三角形から、``Run as __main__''オプションを選ばねばなりません。
このオプションはアプリケーションではなく\emph{ファイル} に関連付けられて
いるので注意してください。とはいえ、このオプションは保存後もそのまま
\emph{残ります}。止めたければ、再度このオプションを選んでください。

\subsection{``Save as'' と ``Save as Applet'' の違い
            \label{IDEapplet}} % ``Save as'' versus ``Save as Applet''
...
標準的な \member{sys.argv} の動作を使います。詳しくは
...
\section{パッケージマネージャ} % The Package Manager
...
の機能として使うこともできます。この場合はメニューから File->Package Manager
を選択します。
