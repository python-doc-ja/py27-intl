...
sub do_cmd_envvar{
    ...
    add_index_entry("kankyouhensuu@環境変数!$envvar@<tt>$envvar</tt>",
                    $ahref);
    add_index_entry("$envvar (環境変数)", $ahref);
...
sub versionnote($$){
    ...
    my $release = next_argument();
    my $text = "バージョン $release で $type された仕様です。";
    if ($explanation) {
        $text = "バージョン $release で $type された仕様:\n$explanation";
    }
    return "\n<span class=\"versionnote\">$text</span>\n" . $_;
}

sub do_cmd_versionadded{
    return versionnote('新たに追加', $_[0]);
}

sub do_cmd_versionchanged{
    return versionnote('変更', $_[0]);
}
...
sub do_cmd_platform{
    ...
    return "\n<p class=\"availability\"><span"
      . "\n class=\"platform\">$platform</span>プラットフォームで利用できます。</p>\n" . $_;
}
...
sub idx_cmd_kwindex($){
    my $str = next_argument();
    add_index_entry("<tt>$str</tt>!キーワード", $_[0]);
    add_index_entry("キーワード!<tt>$str</tt>", $_[0]);
}
...
sub idx_cmd_bifuncindex($){
    my $str = next_argument();
    add_index_entry("<tt class=\"function\">$str()</tt> (組み込み関数)",
                    $_[0]);
}
...
sub define_module($$){
    ...
    $word = "$word " if $word;
    $THIS_MODULE = "$name";
    $INDEX_SUBITEM = "($name モジュール)";
    print "[$name]";
    return make_mod_index_entry(
        "<tt class=\"module\">$name</tt> (${word} モジュール)", 'DEF');
}
...
sub do_cmd_bimodindex{ return my_module_index_helper('組み込み', $_[0]); }
sub do_cmd_exmodindex{ return my_module_index_helper('拡張', $_[0]); }
sub do_cmd_stmodindex{ return my_module_index_helper('標準', $_[0]); }
#     local($_) = @_;
#     my $name = next_argument();
...
sub ref_module_index_helper($$){
    ...
    my $str = next_argument();
    $word = "$word " if $word;
    $str = "<tt class=\"module\">$str</tt> (${word} モジュール)";
...
sub idx_cmd_refbimodindex($){
    return ref_module_index_helper('組み込み', $_[0]); }
sub idx_cmd_refexmodindex($){
    return ref_module_index_helper('拡張', $_[0]);}
sub idx_cmd_refstmodindex($){
    return ref_module_index_helper('標準', $_[0]); }
...
sub do_env_cfuncdesc{
    ...
    if ($result_rc eq '+1') {
        ...
        $rcinfo = '常に <tt class="constant">NULL</tt>';
    }
    if ($rcinfo ne '') {
        $rcinfo = (  "\n<div class=\"refcount-info\">"
                   . "\n  <span class=\"label\">戻り値:</span>"
...
sub handle_classlike_descriptor($$){
    ...
    $idx = make_str_index_entry(
        "<tt class=\"$what\">$THIS_CLASS</tt> ($THIS_MODULE の$what)" );
    $idx =~ s/ \(.*\)//;
...
sub do_env_classdesc{
    return handle_classlike_descriptor($_[0], "クラス");
}
...
sub do_env_classdescstar{
    ...
    $THIS_CLASS = next_argument();
    $idx = make_str_index_entry(
        "<tt class=\"class\">$THIS_CLASS</tt> ($THIS_MODULE のクラス)");
    $idx =~ s/ \(.*\)//;
...
sub do_env_excclassdesc{
    return handle_classlike_descriptor($_[0], "例外");
}
...
sub do_cmd_seemodule{
    ...
    return ('<dl compact="compact" class="seemodule">'
            . "\n    <dt><b><tt class=\"module\">"
            . "<a href=\"module-$key.html\">$module</a></tt>:</b>モジュール</dt>"
            . "\n    <dd>$text$period</dd>\n  </dl>"
            . $_);
}
