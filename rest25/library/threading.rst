...
.. function:: activeCount()
   ...
   Return the number of currently active :class:`Thread` objects. The returned
...
.. function:: enumerate()
   ...
   Return a list of all currently active :class:`Thread` objects. The list includes
...
state to unlocked and returns immediately.  When more than one thread is blocked
...
.. method:: XXX Class.acquire([blocking\ ``= 1``])
...
.. method:: XXX Class.release()
   ...
   Only call this method when the calling thread owns the lock. Do not call this
...
be called when the calling thread has acquired the lock.
...
.. method:: Condition.wait([timeout])
   ...
   Wait until notified or until a timeout occurs. This must only be called when the
...
.. method:: Condition.notify()
   ...
   Wake up a thread waiting on this condition, if any. This must only be called
...
.. method:: Condition.notifyAll()
   ...
   :meth:`notify`, but wakes up all waiting threads instead of one.
