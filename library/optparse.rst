...
:mod:`optparse` では、 ``long`` や ``complex`` といった組み込み型も取り入れています。型の追加は:ref
:`optparse-extending-optparse` で触れています。
...
* スクリプト自体の利用法を表すメッセージを定義します::
  ...
  usage の文字列を指定しない場合、 :mod:`optparse` は型どおりとはいえ気の効いたデフォルト値、
...
パーザにオプションを加えていくにはいくつか方法があります。推奨するのは :ref:`optparse-tutorial` 節で示したような
