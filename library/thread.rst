...
共有するマルチスレッドを制御します。
同期のための単純なロック (別名 :dfn:`mutexes` または バイナリセマフォ
...
.. function:: start_new_thread(function, args[, kwargs])
   ...
   *args* (タプルでなければなりません)の 関数 *function* を実行します。
...
* :func:`sys.exit` を呼び出す、あるいは :exc:`SystemExit` 例外を送出す
  ることは、 :func:`exit` を呼び出すことと同じです。
