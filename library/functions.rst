...
.. function:: compile(source, filename, mode[, flags[, dont_inherit]])
   ...
   *source* は、文字列と AST オブジェクトのどちらでもかまいません。
   ...
   オプションの引数 *flags* および *dont_inherit* (Python 2.2 で 新た
...
.. function:: input([prompt])
   ...
   .. warning::
      ...
      この関数はユーザのエラーに対して安全ではありません ! この関数 で
...
.. function:: property([fget[, fset[, fdel[, doc]]]])
   ...
   *doc* がもし与えられたならばそれがプロパティ属性のドキュメント文字
   ...
   のようにすると、:meth:`voltage` が同じ名前の読み取り専用属性 の
   "getter" になります。
...
.. function:: reload(module)
   ...
   * 以前のオブジェクトが (外部の他のモジュールなどからの) 参照を受け
   ...
   ようには設計されておらず、再ロードされた場合には何らかの理由で 失敗
   するかもしれません。
...
.. function:: setattr(object, name, value)
   ...
   に 関連付けますが、指定したオブジェクトにおいて可能な場合に限ります。
...
.. function:: slice([start,] stop[, step])
   ...
   値 (または 標準の値) を返します。これらの値には、その他のはっきりと
   した機能は ありません; しかしながら、これらの値は Numerical Python
...
.. function:: staticmethod(function)
   ...
   *function* の静的メソッドを返します。
   ...
   しくは :ref:`function` リファレンスマニュアル の 7 章にある関数定義
   についての説明を参照してください。
...
.. function:: unichr(i)
   ...
   を返します。この関数は Unicode 文字列に対する :func:`ord` の逆 です。
